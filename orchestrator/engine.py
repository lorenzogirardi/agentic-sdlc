    ...
    while True:
        try:
            ...
            for card in cards:
                ...
                if github and args.mode == "pr":
                    ...
                    try:
                        await github.create_branch(
                            gh_owner, gh_repo, branch_name, task.base_branch,
                        )
                    except Exception as exc:
                ...
                prefix = {"PASS": "PASS", "PASS_WITH_WARNINGS": "PASS*",
                          "BLOCKED": "BLOCKED", "REQUIRES_HUMAN_APPROVAL": "REVIEW"}

                await trello.update_card_fields(
                    card_id, name=f"[{prefix.get(verdict, verdict)}] {card_name}",
                )
