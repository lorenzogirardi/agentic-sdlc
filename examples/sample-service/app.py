...
async def burning_ship(
    ...
    points = _burning_ship_set(w, h, xmin, xmax, ymin, ymax, max_iter)
    return JSONResponse(
        content={
            "type": "burning-ship",
            "parameters": {"xmin": xmin, "xmax": xmax, "ymin": ymin, "ymax": ymax, "max_iterations": max_iter},
            "width": w,
            "height": h,
            "points": points,
        }
...
def _burning_ship_set(w: int, h: int, xmin: float, xmax: float, ymin: float, ymax: float, max_iter: int) -> list[list[int]]:
    result: list[list[int]] = []
    for py in range(h):
        row: list[int] = []
        for px in range(w):
            x = xmin + (xmax - xmin) * px / (w - 1) if w > 1 else xmin
            y = ymin + (ymax - ymin) * py / (h - 1) if h > 1 else ymin
            zx = x
            zy = y
            iteration = 0
            while zx * zx + zy * zy < 4.0 and iteration < max_iter:
