...
def _tricorn_set(w: int, h: int, xmin: float, xmax: float, ymin: float, ymax: float, max_iter: int) -> list[list[int]]:
    result: list[list[int]] = []
    for py in range(h):
        row: list[int] = []
        c_imag = ymin + (py / h) * (ymax - ymin)
        for px in range(w):
            c_real = xmin + (px / w) * (xmax - xmin)
            zx = 0.0
            zy = 0.0
            iteration = 0
            while zx * zx + zy * zy < 4.0 and iteration < max_iter:
